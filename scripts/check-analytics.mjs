...
async function run({ endpoint = 'https://example.goatcounter.com', hostname = 'bowenyu066.github.io', search = '', lang = 'en', fail = false, count = '1,234', framed = false } = {}) {
  ...
  vm.runInNewContext(source, {
    ...
    fetch: async url => { requests.push(url); return { ok: !fail, json: async () => ({ count }) }; },
  });
...
assert.equal(ok.requests[0], 'https://example.goatcounter.com/counter/%2Fblogs%2Fexample%2F.json');
...
for (const options of [{ fail: true }, { count: '<img>' }, { count: null }]) assert.equal((await run(options)).counter.hidden, true);
